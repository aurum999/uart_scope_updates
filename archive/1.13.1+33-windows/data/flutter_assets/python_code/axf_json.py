...
def run_list(
    ...
    *,
    ...
    trace_timing_enabled: bool = False,
) -> int:
    total_start = time.perf_counter()
    variables = parse_axf_globals(
        axf_path,
        only_external=only_external,
        trace=trace_timing_enabled,
    )
    encode_start = time.perf_counter()
    if output_format == "binary":
        payload, string_count = encode_variables_binary(variables)
        ...
        trace_timing(
            ...
            f"bytes={len(payload)} "
            f"encode={encode_ms:.2f}ms "
            f"write={write_ms:.2f}ms "
            f"total={elapsed_ms(total_start):.2f}ms",
        )
    else:
        text = json.dumps(variables, ensure_ascii=False, separators=(",", ":"))
        encode_ms = elapsed_ms(encode_start)
        if output:
            write_start = time.perf_counter()
        ...
        trace_timing(
            ...
            f"format=json "
            f"variables={len(variables)} "
            f"jsonBytes={len(text.encode('utf-8'))} "
            f"jsonDump={encode_ms:.2f}ms "
            f"write={write_ms:.2f}ms "
            f"total={elapsed_ms(total_start):.2f}ms",
        )
    return 0
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if argv and argv[0] == "list":
        ...
        parser.add_argument("--trace-timing", action="store_true")
        ...
        return run_list(
            ...
            trace_timing_enabled=args.trace_timing,
        )
